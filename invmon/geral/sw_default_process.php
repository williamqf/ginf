...
$data['software'] = (isset($post['software']) && !empty($post['software']) ? noHtml($post['software']) : "");
...
if ($data['action'] == 'new') {
    ...
    $sql = "SELECT * FROM sw_padrao WHERE swp_sw_cod = '" . $data['software'] . "' ";
    $res = $conn->query($sql);
    if ($res->rowCount()) {
        $data['success'] = false; 
        $data['field_id'] = "software";
    ...
    $sql = "DELETE FROM sw_padrao WHERE swp_sw_cod = '" . $data['cod'] . "'";
