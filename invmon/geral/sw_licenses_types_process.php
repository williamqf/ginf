...
$data['license_name'] = (isset($post['license_name']) && !empty($post['license_name']) ? noHtml($post['license_name']) : "");
...
if ($data['action'] == 'new') {
    ...
    $sql = "SELECT * FROM licencas WHERE lic_desc = '" . $data['license_name'] . "' ";
    $res = $conn->query($sql);
    if ($res->rowCount()) {
        $data['success'] = false; 
        $data['field_id'] = "license_name";
    ...
    $sql = "SELECT lic_cod FROM licencas WHERE lic_desc = '" . $data['license_name'] . "' AND lic_cod <> '" . $data['cod'] . "'";
    $res = $conn->query($sql);
    if ($res->rowCount()) {
        $data['success'] = false; 
        $data['field_id'] = "license_name";
    ...
    $sql = "UPDATE licencas SET 
				lic_desc = '" . $data['license_name'] . "'
            WHERE lic_cod = '" . $data['cod'] . "'";
    ...
    $sqlFindPrevention = "SELECT * FROM softwares WHERE soft_tipo_lic = ".$data['cod']."";
    ...
    $sql = "DELETE FROM licencas WHERE lic_cod = '" . $data['cod'] . "'";
